...
#                           make_image_summary=True,
#                           attn_stick=None,
#                           layer=None, hparams=None):
#     with tf.variable_scope(
#             name, default_name="dot_product_attention", values=[q, k, v]):
#         reduce_dim = q.get_shape()[-1].value
#         xinit = tf.contrib.layers.xavier_initializer()
#         w1 = tf.get_variable('w', shape=[reduce_dim, reduce_dim], dtype=tf.float32, initializer=xinit)
#         w2 = tf.get_variable('w', shape=[reduce_dim, reduce_dim], dtype=tf.float32, initializer=xinit)
#         w3 = tf.get_variable('w', shape=[reduce_dim, reduce_dim], dtype=tf.float32, initializer=xinit)
#
#         tf.nn.conv1d()
#
#         return None
  ...
  with tf.variable_scope(
    ...
    if layer is not None and attn_stick is not None:
        if 'tuzhaopeng' in hparams.cov_mode and 'wd_attn' in hparams.cov_mode:
            ...
            dim_weight_trans = tf.get_variable('dim_weight_trans',
                                    shape=[1, q.get_shape()[-1].value, q.get_shape()[-1].value],
